...
public class DialogUtil {
    ...
    public static void showWhitSubtitleCanntDissMiss(Context mContext, String content, String Subtitle, String left, String right, View.OnClickListener leftListener, View.OnClickListener rightListener) {
        dialog = new Dialog(mContext, R.style.DialogStyle);
        View view = LayoutInflater.from(mContext).inflate(R.layout.dialog_no_title, null);
        dialog.setContentView(view);
    ...
    public static void showDeleteDialog(Context mContext, View.OnClickListener listener) {
        ...
        lp.width = UiUtils.getScreenWidth(mContext); //设置宽度
    ...
    public static void showLoadingDialog(Context mContext, String showText) {
        @SuppressLint("InflateParams")
        FrameLayout layout = (FrameLayout) LayoutInflater.from(mContext).inflate(R.layout.loading_dialog, null);
        dialog = new Dialog(mContext, R.style.loading_dialog);// 创建自定义样式dialog
        dialog.setCanceledOnTouchOutside(true);
        dialog.show();
        dialog.setCancelable(true);
        WindowManager.LayoutParams params = dialog.getWindow().getAttributes();
        params.width = UiUtils.getScreenWidth(mContext);
        dialog.getWindow().setAttributes(params);
        dialog.getWindow().setContentView(layout, params);
        TextView textView = (TextView) layout.findViewById(R.id.tipTextView);
        textView.setText(showText);
        AVLoadingIndicatorView loadingIndicatorView = (AVLoadingIndicatorView) layout.findViewById(R.id.loadingView);
        loadingIndicatorView.setIndicator("CubeTransitionIndicator");
        loadingIndicatorView.setIndicatorColor(Color.parseColor("#22ccbf"));
